...
function EndlessGameState:resetRun()
    ...
    if _G.Game.SaveSystem then
        -- HP upgrade
        local hpLevel = _G.Game.SaveSystem:getPermanentUpgradeLevel("hp")
        if hpLevel > 0 then
            baseMaxHp = baseMaxHp + (25 * hpLevel) -- +25 HP per level
        end

        -- Damage upgrade
        local dmgLevel = _G.Game.SaveSystem:getPermanentUpgradeLevel("dmg")
        if dmgLevel > 0 then
            baseDamage = baseDamage + (0.2 * dmgLevel) -- +0.2 damage per level
        end

        -- Speed upgrade
        local speedLevel = _G.Game.SaveSystem:getPermanentUpgradeLevel("spd")
        if speedLevel > 0 then
            baseSpeed = baseSpeed + (20 * speedLevel) -- +20 speed per level
        end
    end

    -- Calculate base cooldown with upgrades
    local baseShootCd = 0.18
    if _G.Game.SaveSystem then
        local cdrLevel = _G.Game.SaveSystem:getPermanentUpgradeLevel("cdr")
        if cdrLevel > 0 then
            baseShootCd = baseShootCd * (1.0 - (0.05 * cdrLevel)) -- -5% cooldown per level
        end
    end
    ...
    -- Store base damage for weapon calculations
    self.bulletDamage                = baseDamage
    ...
    self.autoFireActive              = _G.Game.SaveSystem and _G.Game.SaveSystem:hasPermanentUpgrade("auto_fire") or
        false
...
function EndlessGameState:update(dt)
    ...
    if dx ~= 0 or dy ~= 0 then
        local len = math.sqrt(dx * dx + dy * dy); dx, dy = dx / len, dy / len
    end
    ...
    if _G.Game.SaveSystem then
        local speedLevel = _G.Game.SaveSystem:getPermanentUpgradeLevel("spd")
        if speedLevel > 0 then
            currentSpeed = currentSpeed + (20 * speedLevel) -- +20 speed per level
        end
    end
        ...
        table.insert(self.bullets,
            {
                x = self.player.x,
                y = self.player.y,
                vx = math.cos(ang) * currentProjectileSpeed,
                vy = math.sin(ang) *
    ...
    while self.spawnAcc >= 1 do
        ...
        local ex, ey
        if side == 1 then
            ex, ey = -20, love.math.random(0, h)
        elseif side == 2 then
            ex, ey = w + 20, love.math.random(0, h)
        elseif side == 3 then
            ex, ey = love.math.random(0, w), -20
        else
            ex, ey = love.math.random(0, w), h + 20
        end
        ...
        local e = {
            ...
            hp = math.max(1,
                math.floor(base.hp * hpMul + 0.5)),
            dmg = math.floor(base.dmg * dmgMul + 0.5),
            color = base.color
        }
    ...
    for ei = #self.enemies, 1, -1 do
        ...
        if dist2(e.x, e.y, self.player.x, self.player.y) < (e.r + self.player.r) * (e.r + self.player.r) then
            ...
            if self.player.hp <= 0 then
                self.player.hp = 0; self.gameOver = true
                local t = self.timeSurvived
                local p = _G.Game.PROFILE and _G.Game.PROFILE.player
                if p then
                    p.bestSurvivalTime = math.max(p.bestSurvivalTime or 0, t); _G.Game.SaveSystem:save(_G.Game.PROFILE)
                end
                break
            end
        end
...
function EndlessGameState:draw()
    ...
    for _, e in ipairs(self.enemies) do
        love.graphics.setColor(e.color); love.graphics.circle("fill", e.x, e.y, e.r)
    end
    ...
    if self.gameOver then
        ...
        love.graphics.setColor(self.colors.text); love.graphics.printf("Restart (R)", ww * 0.5 - 80, hh * 0.56, 160,
            "center")
    end
...
function EndlessGameState:resize(w, h)
    if self.ui then
        self.ui:resize(w, h); self:layout(w, h)
    end
...
function EndlessGameState:buildLayout(w, h)
    ...
    self.btnBack = UIElement({
        ...
        callbackName = "endless:backToHub",
        hoverCallbackName = "endless:buttonHover",
        pressCallbackName = "endless:buttonPress",
        releaseCallbackName = "endless:buttonRelease",
        zIndex = 2,
        isFocusable = false
    })
...
function EndlessGameState:layout(w, h)
    if self.btnBack then
        self.btnBack:setPosition(24, h - 24); self.btnBack:setSize(220, 42)
    end
end
